...
async function probeCdpPipe(candidate, { timeoutMs }) {
  ...
  const child = spawn(candidate.command, launch.args, launch.spawnOptions);
  let stderr = "";
  child.stderr.setEncoding("utf8");
  child.stderr.on("data", (chunk) => {
    stderr += chunk;
  });
  const client = new CdpPipeClient(child.stdio[4], child.stdio[3], {
    timeoutMs
  });
  try {
    await client.connect();
    const version = await verifyCdpClient(client);
    return { transport: "pipe", version };
  } catch (error) {
...
async function probeCdpWebSocket(candidate, { timeoutMs }) {
  const profile = await mkdtemp(path.join(os.tmpdir(), "ppt-browser-probe-"));
  const launch = buildBrowserLaunchOptions({
    profile,
    args: [
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (endpoint) {
        client = await retryCdpConnection(async () => {
          const candidateClient = new CdpWebSocketClient(endpoint, { timeoutMs });
          try {
        ...
        const version = await verifyCdpClient(client);
        return { transport: "websocket", endpoint, version };
      }
...
export async function defaultProbeCdp(
  candidate,
  { timeoutMs = 10000 } = {}
) {
  const failures = [];
  try {
    return await probeCdpPipe(candidate, { timeoutMs });
  } catch (error) {
    failures.push(`pipe: ${error.message}`);
  }
  try {
    return await probeCdpWebSocket(candidate, { timeoutMs });
  } catch (error) {
    failures.push(`websocket: ${error.message}`);
...
export async function probeBrowserCandidate(
  ...
  try {
    ...
    return {
      ...
      usable: true,
      cdp: true,
      cdpTransport: cdp?.transport ?? null,
      endpointObserved: Boolean(cdp?.endpoint),
      cdpProduct: cdp?.version?.product ?? null
    };
  } catch (error) {
    return {
      ...parsed,
