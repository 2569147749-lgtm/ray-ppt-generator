...
export class CdpClient {
  ...
  async connect() {
    ...
    await new Promise((resolve, reject) => {
      ...
      const onError = () =>
        finish(reject, new Error("Chrome DevTools connection failed."));
      const onClose = () =>
        finish(reject, new Error("Chrome DevTools connection closed before opening."));
    ...
    this.socket.addEventListener("error", () => {
      this.failPending(new Error("Chrome DevTools connection failed."));
    });
...
export async function renderScreenshots({
  ...
  try {
    ...
    client = new CdpClient(pageEndpoint, { timeoutMs });
    await guardBrowserOperation(child, client.connect());
    await guardBrowserOperation(child, client.send("Page.enable"));
    await guardBrowserOperation(child, client.send("Runtime.enable"));
    const renderSessionId = `chrome-${child.pid}`;
    const results = [];
    ...
    const renderSessionId = child.pid ? `chrome-${child.pid}` : null;
    return tasks.map(() => ({
      error: error.message,
      renderSessionId
    }));
